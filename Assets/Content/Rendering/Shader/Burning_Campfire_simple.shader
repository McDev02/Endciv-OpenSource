...
Shader "ENDCIV/Burning Camprfire simple" {
	Properties{
		_Spec("Specular Power", Range(0.00, 1)) = 0.3
		_Shininess("Glossiness", Range(0.001, 1)) = 0.078125
		_MainTex("Base (RGB) BurningMask (A)", 2D) = "white" {}
		_BumpMap("Normalmap", 2D) = "bump" {}
		_BurntColor("Burnt Color", color) = (1,0.5,0,1)
...
#pragma target 3.0
	...
	void surf(Input IN, inout SurfaceOutputStandardSpecular o) {
		...
		fixed3 tex = idle.rgb * lerp(float3(1,1,1), _BurntColor.rgb, mask);

		half3 glowMask = pow(idle.a*saturate((IN.yDirection + _GlowRate)*_GlowRate),2);
