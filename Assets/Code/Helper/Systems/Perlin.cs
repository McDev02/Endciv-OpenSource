...
namespace Endciv
{
	...
	*/
	...
	public class Perlin
	{
		...
		private Vector3[] g3 = new Vector3[B + B + 2];
		private Vector2[] g2 = new Vector2[B + B + 2];
		...
		public float Noise(float value)
		{
			int bx0, bx1;
			float rx0, rx1, sx, u, v;
			setup(value, out bx0, out bx1, out rx0, out rx1);
		...
		public float Noise(float x, float y)
		{
			...
			u = at2(rx0, ry0, ref g2[b00]);
			v = at2(rx1, ry0, ref g2[b10]);
			a = lerp(sx, u, v);

			u = at2(rx0, ry1, ref g2[b01]);
			v = at2(rx1, ry1, ref g2[b11]);
			b = lerp(sx, u, v);
		...
		public float Noise(float x, float y, float z)
		{
			...
			u = at3(rx0, ry0, rz0, ref g3[b00 + bz0]);
			v = at3(rx1, ry0, rz0, ref g3[b10 + bz0]);
			a = lerp(t, u, v);

			u = at3(rx0, ry1, rz0, ref g3[b01 + bz0]);
			v = at3(rx1, ry1, rz0, ref g3[b11 + bz0]);
			b = lerp(t, u, v);

			c = lerp(sy, a, b);

			u = at3(rx0, ry0, rz1, ref g3[b00 + bz1]);
			v = at3(rx1, ry0, rz1, ref g3[b10 + bz1]);
			a = lerp(t, u, v);

			u = at3(rx0, ry1, rz1, ref g3[b01 + bz1]);
			v = at3(rx1, ry1, rz1, ref g3[b11 + bz1]);
			b = lerp(t, u, v);
		...
		private void Generate(int seed)
		{
			...
			const float InvB = 1f / B;
			for (i = 0; i < B; i++)
			{
				p[i] = i;
				g1[i] = (rnd.Next(B + B) - B) * InvB;

				v2.x = (rnd.Next(B + B) - B) * InvB;
				v2.y = (rnd.Next(B + B) - B) * InvB;
				v2.Normalize();
				g2[i] = v2;

				v3.x = (rnd.Next(B + B) - B) * InvB;
				v3.y = (rnd.Next(B + B) - B) * InvB;
				v3.z = (rnd.Next(B + B) - B) * InvB;
				v3.Normalize();
				g3[i] = v3;
			}
		...
		private static void setup(float value, out int b0, out int b1, out float r0, out float r1)
		{
			float t = value + N;
			b0 = ((int)t) & BM;
			b1 = (b0 + 1) & BM;
		...
		private static float at2(float rx, float ry, ref Vector2 v2)
		{
			return rx * v2.x + ry * v2.y;
		}

		private static float at3(float rx, float ry, float rz, ref Vector3 v3)
		{
			return rx * v3.x + ry * v3.y + rz * v3.z;
		}
	}
